...
| ST-01 | Lectura de humedad en tiempo real | Mostrar el nivel actual de humedad del sustrato. | **Scenario 1: Visualización de humedad actual en la app**<br>Dado que el usuario abrió la sección "Humedad Actual" en la app<br>Cuando el sensor envía el porcentaje de humedad en tiempo real<br>Entonces el Sistema muestra el valor en % en la app<br>Y el Sistema verifica que el error máximo sea de ±5%.<br><br>**Scenario 2: Actualización periódica del valor de humedad**<br>Dado que el usuario se encuentra en la vista de humedad<br>Cuando transcurren 5 minutos desde la última lectura<br>Entonces el Sistema actualiza automáticamente el porcentaje de humedad<br>Y el Sistema muestra la nueva lectura en la app y la plataforma web. | EP-01 |
| ST-02 | Historial de humedad | Guardar registros diarios de humedad. | **Scenario 1: Registro diario de humedad**<br>Dado que el sistema recibe mediciones de humedad durante el día<br>Cuando se completa el día calendario<br>Entonces el Sistema guarda los valores diarios en el historial<br>Y el Sistema los muestra en gráficos por día, semana y mes.<br><br>**Scenario 2: Exportación del historial**<br>Dado que el usuario accede a la sección "Historial de Humedad"<br>Cuando selecciona la opción "Exportar en .csv"<br>Entonces el Sistema genera un archivo .csv con los registros almacenados<br>Y el Sistema permite descargar el archivo en la app o web. | EP-01 |
...
| ST-03 | Lectura de luz | Mostrar luminosidad actual. | **Scenario 1: Mostrar luminosidad actual**<br>Dado que el usuario está en la sección "Luz Actual"<br>Cuando el sensor envía la lectura de luminosidad<br>Entonces el Sistema muestra el valor en lux<br>Y verifica que la precisión sea de ±10%.<br><br>**Scenario 2: Actualización periódica de luminosidad**<br>Dado que la app se encuentra mostrando la lectura de luz<br>Cuando transcurren 10 minutos desde la última actualización<br>Entonces el Sistema solicita una nueva lectura al sensor<br>Y muestra el nuevo valor en lux en la app y web. | EP-02 |
| ST-04 | Alertas de luz insuficiente | Enviar alerta si la luz está fuera del rango óptimo configurado. | **Scenario 1: Alerta por luz bajo el umbral**<br>Dado que el usuario configuró un rango mínimo de luminosidad<br>Cuando el sensor detecta que la luz está por debajo del rango óptimo<br>Entonces el Sistema envía una notificación push en menos de 30 segundos<br>Y registra el evento en el historial de alertas.<br><br>**Scenario 2: Alerta por luz excesiva**<br>Dado que el usuario configuró un rango máximo de luminosidad<br>Cuando la lectura del sensor excede el límite superior<br>Entonces el Sistema envía una notificación push al usuario en menos de 30 segundos<br>Y muestra en la app el valor de lux detectado. | EP-02 |
...
| ST-05 | Lectura de temperatura | Visualizar temperatura en tiempo real. | **Scenario 1: Visualización de temperatura actual**<br>Dado que el usuario abrió la sección "Temperatura Actual"<br>Cuando el sensor envía la lectura de temperatura<br>Entonces el Sistema muestra el valor en °C<br>Y verifica que esté dentro del rango 0–50 °C.<br><br>**Scenario 2: Actualización periódica de temperatura**<br>Dado que el sistema recibe valores de temperatura cada 5 minutos<br>Cuando se recibe una nueva lectura<br>Entonces la app actualiza el valor mostrado en tiempo real<br>Y registra la lectura en memoria temporal. | EP-03 |
| ST-06 | Historial de temperatura | Guardar histórico diario/semanal. | **Scenario 1: Registro diario del historial**<br>Dado que existe medición continua durante el día<br>Cuando termina el día calendario<br>Entonces el Sistema almacena el promedio, mínimo y máximo diario<br>Y los muestra en gráficos consultables en la app.<br><br>**Scenario 2: Consulta del historial**<br>Dado que el usuario accede al módulo "Historial de Temperatura"<br>Cuando selecciona un rango de días<br>Entonces el Sistema muestra gráficos del periodo seleccionado<br>Y presenta la variación diaria de manera detallada. | EP-03 |
...
| ST-07 | Configurar umbral de riego | Permitir al usuario fijar % de humedad mínimo. | **Scenario 1: Configuración de umbral por el usuario**<br>Dado que el usuario abrió la sección "Configuración de Riego"<br>Cuando ingresa un porcentaje mínimo de humedad<br>Entonces el Sistema guarda el nuevo umbral<br>Y confirma la configuración mediante un mensaje en pantalla.<br><br>**Scenario 2: Validación de umbral ingresado**<br>Dado que el usuario ingresa un valor de umbral fuera del rango permitido<br>Cuando hace clic en "Guardar"<br>Entonces el Sistema muestra un mensaje de error<br>Y no permite guardar la configuración. | EP-04 |
| ST-08 | Activar riego automático | Riego se activa cuando humedad < umbral. | **Scenario 1: Activación automática por humedad baja**<br>Dado que el usuario configuró un umbral mínimo de humedad<br>Cuando el sensor detecta un valor inferior al umbral<br>Entonces el Sistema activa automáticamente la bomba de riego<br>Y registra en la app un log de inicio del riego.<br><br>**Scenario 2: Finalización automática del riego**<br>Dado que la bomba de riego está activa<br>Cuando la humedad alcanza nuevamente el umbral mínimo establecido<br>Entonces el Sistema detiene la bomba<br>Y registra un log de finalización de riego en la app. | EP-04 |
| ST-09 | Riego manual desde app | Permitir riego inmediato desde app. | **Scenario 1: Activación manual**<br>Dado que el usuario está en la sección "Riego Manual"<br>Cuando presiona el botón "Regar ahora"<br>Entonces el Sistema activa inmediatamente la bomba<br>Y muestra un mensaje de "Riego iniciado".<br><br>**Scenario 2: Confirmación de finalización**<br>Dado que se activó el riego manual<br>Cuando la acción termina su ciclo predefinido<br>Entonces el Sistema muestra un mensaje de "Riego completado"<br>Y registra el evento en el historial de riegos. | EP-04 |
...
| ST-10 | Alerta de humedad crítica | Enviar notificación si humedad < umbral de seguridad. | **Scenario 1: Envío de alerta por humedad crítica**<br>Dado que el sistema monitorea la humedad continuamente<br>Cuando la humedad cae por debajo del umbral crítico<br>Entonces el Sistema envía una notificación push en menos de 30 segundos<br>Y muestra en pantalla el valor detectado.<br><br>**Scenario 2: Registro del evento**<br>Dado que ocurrió una alerta de humedad crítica<br>Cuando el usuario revisa el historial de alertas<br>Entonces el Sistema muestra el evento registrado<br>Y permite visualizar fecha y hora del evento. | EP-05 |
| ST-11 | Alerta de temperatura extrema | Avisar si temperatura excede rangos definidos. | **Scenario 1: Alerta por temperatura alta**<br>Dado que el sensor detecta la temperatura actual<br>Cuando supera el rango máximo permitido<br>Entonces el Sistema envía una alerta inmediata<br>Y muestra la temperatura exacta en la notificación.<br><br>**Scenario 2: Alerta por temperatura baja**<br>Dado que existe un rango mínimo configurado<br>Cuando la temperatura cae por debajo de dicho rango<br>Entonces el Sistema envía una notificación push<br>Y marca el evento como "temperatura extrema" en el historial. | EP-05 |
| ST-12 | Confirmación de envío | Registrar confirmación de que la alerta fue entregada. | **Scenario 1: Registro de entrega de alerta**<br>Dado que se envió una notificación al usuario<br>Cuando el sistema recibe confirmación del proveedor de envío<br>Entonces marca el estado como "Enviada"<br>Y actualiza el panel de alertas.<br><br>**Scenario 2: Error en el envío**<br>Dado que se intentó enviar una alerta<br>Cuando el proveedor devuelve un error<br>Entonces el Sistema muestra estado "Error de envío"<br>Y registra el fallo en el panel. | EP-05 |
...
| ST-13 | Registro y login de usuario | Permitir crear cuenta y autenticarse. | **Scenario 1: Registro exitoso**<br>Dado que el usuario se encuentra en el formulario de registro<br>Cuando ingresa email válido y contraseña<br>Entonces el Sistema crea la cuenta<br>Y muestra un mensaje de confirmación.<br><br>**Scenario 2: Error en login**<br>Dado que el usuario está en la pantalla de login<br>Cuando ingresa credenciales incorrectas<br>Entonces el Sistema muestra un mensaje de error claro<br>Y permanece en la misma pantalla. | EP-06 |
| ST-14 | Dashboard de métricas | Mostrar humedad, luz, temperatura y riego. | **Scenario 1: Visualización de métricas en tiempo real**<br>Dado que el usuario accede al dashboard<br>Cuando el sistema recibe datos de humedad, luz, temperatura y riego<br>Entonces el Sistema muestra las métricas en tiempo real<br>Y actualiza los gráficos correspondientes.<br><br>**Scenario 2: Error de obtención de datos**<br>Dado que el dashboard requiere datos de sensores<br>Cuando el Sistema no puede obtener una lectura<br>Entonces muestra un mensaje de "Datos no disponibles"<br>Y conserva la última lectura válida mostrada. | EP-06 |
| ST-15 | Comunidad de usuarios | Compartir consejos y fotos. | **Scenario 1: Publicación de contenido**<br>Dado que el usuario accede a la sección Comunidad<br>Cuando publica un consejo o foto<br>Entonces el Sistema registra la publicación<br>Y la muestra inmediatamente en el foro interno.<br><br>**Scenario 2: Comentario en una publicación**<br>Dado que el usuario observa una publicación existente<br>Cuando escribe un comentario<br>Entonces el Sistema guarda el comentario<br>Y lo muestra debajo de la publicación. | EP-06 |
...
| ST-16 | API para integraciones | Ofrecer endpoints REST/MQTT para terceros. | **Scenario 1: Acceso a endpoint seguro**<br>Dado que un tercero desea consumir la API<br>Cuando envía una solicitud con token válido<br>Entonces el Sistema permite el acceso<br>Y devuelve los datos solicitados.<br><br>**Scenario 2: Token inválido**<br>Dado que un tercero realiza una solicitud<br>Cuando envía un token inválido o expirado<br>Entonces el Sistema rechaza la petición<br>Y devuelve un mensaje de error de autenticación. | EP-07 |
| ST-17 | Compatibilidad con asistentes de voz | Enviar/recibir comandos básicos. | **Scenario 1: Recepción de comando de voz**<br>Dado que el usuario configuró su asistente de voz<br>Cuando envía un comando como "Consultar humedad"<br>Entonces el Sistema procesa la solicitud<br>Y responde con los valores actuales de humedad.<br><br>**Scenario 2: Envío de comando hacia la maceta**<br>Dado que el usuario usa un asistente compatible<br>Cuando ordena "Activa el riego"<br>Entonces el Sistema recibe el comando<br>Y ejecuta la acción en la maceta inteligente. | EP-07 |
...
| ST-18 | Backup en la nube | Guardar configuraciones y métricas. | **Scenario 1: Backup diario automático**<br>Dado que el día calendario llega a su fin<br>Cuando el proceso programado se ejecuta<br>Entonces el Sistema realiza un respaldo automático<br>Y guarda métricas y configuraciones en la nube.<br><br>**Scenario 2: Restaurar desde backup**<br>Dado que existe un respaldo previo<br>Cuando el usuario selecciona "Restaurar configuración"<br>Entonces el Sistema recupera los datos<br>Y restaura las configuraciones anteriores. | EP-08 |
| ST-19 | Calibración de sensores | Permitir calibración desde app. | **Scenario 1: Iniciar calibración**<br>Dado que el usuario abre la sección "Calibración"<br>Cuando presiona "Iniciar calibración"<br>Entonces el Sistema muestra una guía paso a paso<br>Y comienza el proceso de ajuste del sensor.<br><br>**Scenario 2: Confirmar calibración**<br>Dado que el proceso de calibración ha finalizado<br>Cuando el usuario confirma la lectura obtenida<br>Entonces el Sistema guarda los nuevos parámetros<br>Y muestra un mensaje de calibración exitosa. | EP-08 |
| ST-20 | Notificación de mantenimiento | Avisar cuando se requiera limpieza/batería. | **Scenario 1: Aviso por limpieza necesaria**<br>Dado que el sistema detecta acumulación de suciedad o reducción de rendimiento<br>Cuando la condición alcanza nivel crítico<br>Entonces el Sistema envía un push y correo de recordatorio<br>Y registra el evento en el historial de mantenimiento.<br><br>**Scenario 2: Aviso por batería baja**<br>Dado que la batería de la maceta se encuentra en nivel mínimo<br>Cuando el sensor reporta carga inferior al umbral<br>Entonces el Sistema envía un aviso al usuario<br>Y recomienda acciones para mantener el equipo operativo. | EP-08 |
| ST-21 | Gestión de usuarios y roles | Permitir varios usuarios por maceta con permisos. | **Scenario 1: Añadir usuario invitado**<br>Dado que el usuario administrador accede a "Gestión de usuarios"<br>Cuando ingresa el email de un nuevo invitado<br>Entonces el Sistema agrega al usuario con rol invitado<br>Y le otorga permisos limitados.<br><br>**Scenario 2: Cambio de rol**<br>Dado que existe un usuario invitado<br>Cuando el administrador modifica su rol a "Administrador"<br>Entonces el Sistema actualiza los permisos asignados<br>Y muestra la confirmación de cambio de rol. | EP-08 |
| HW-01 | Prototipo físico operativo | Como equipo técnico, quiero un prototipo de hardware completamente ensamblado y funcional | **Scenario 1: Ensamblaje completo**<br>Dado que tenemos todos los componentes (ESP32, sensores, bomba, LEDs)<br>Cuando se ensamblan según especificaciones<br>Entonces el dispositivo se enciende correctamente<br>Y todos los componentes son detectados.<br><br>**Scenario 2: Pruebas de campo 48h**<br>Dado que el prototipo está ensamblado<br>Cuando se ejecutan pruebas continuas durante 48 horas<br>Entonces el sistema mantiene conectividad estable<br>Y envía datos sin interrupciones. | — |
| INFRA-01 | Pipeline CI/CD automatizado | Como desarrollador, quiero automatizar el despliegue del backend | **Scenario 1: Despliegue automático**<br>Dado que existe código nuevo en la rama main<br>Cuando se hace push al repositorio<br>Entonces GitHub Actions ejecuta el pipeline<br>Y despliega automáticamente en Azure.<br><br>**Scenario 2: Rollback automático**<br>Dado que el despliegue falla<br>Cuando se detectan errores críticos<br>Entonces el sistema revierte automáticamente a la versión anterior<br>Y notifica al equipo técnico. | — |
| DOC-01 | Documentación técnica completa | Como desarrollador, quiero documentación actualizada de la API | **Scenario 1: Swagger actualizado**<br>Dado que existen endpoints implementados<br>Cuando se actualiza la documentación<br>Entonces Swagger refleja todos los endpoints<br>Y los ejemplos de uso son funcionales.<br><br>**Scenario 2: Guía de despliegue**<br>Dado que existe un nuevo miembro del equipo<br>Cuando lee la guía de despliegue<br>Entonces puede configurar el entorno local<br>Y ejecutar la aplicación sin asistencia. | — |
...
| 20 | HW-01 | Prototipo físico operativo | Como equipo técnico, quiero un prototipo de hardware completamente ensamblado y funcional. | 8 |
| 21 | INFRA-01 | Pipeline CI/CD automatizado | Como desarrollador, quiero automatizar el despliegue del backend. | 5 |
| 22 | DOC-01 | Documentación técnica completa | Como desarrollador, quiero documentación actualizada de la API. | 3 |
...
Validación Integral del Ecosistema Digital y Hardware: El desarrollo completo de MaceTy, desde la arquitectura de software hasta la integración del prototipo físico IoT, las validaciones con usuarios reales y la documentación técnica final, confirma que el proyecto trasciende la simple automatización de riego. MaceTy representa un ecosistema digital end-to-end funcional que conecta hardware (ESP32 + sensores), microservicios de backend, aplicación web optimizada y experiencia de usuario validada. La integración exitosa demostrada en los tres sprints —incluyendo comunicación MQTT bidireccional, dashboard en tiempo real y sistema de alertas contextuales— valida técnica y comercialmente la propuesta de valor: resolver la dificultad de cuidar plantas en entornos urbanos mediante tecnología IoT accesible y sostenible.

Experiencia de Usuario como Diferenciador Estratégico Validado: Las seis entrevistas de validación realizadas durante la fase TB2/TF, combinadas con la evaluación heurística exhaustiva, revelaron que los usuarios no solo valoran la automatización, sino especialmente la claridad informativa, confianza en las recomendaciones y diseño intuitivo. El feedback cuantitativo (calificaciones promedio de 4.5/5) y cualitativo (solicitudes de identificación de plantas por foto, tooltips explicativos y contexto en alertas) posiciona la experiencia de usuario superior como el principal diferenciador frente a competidores internacionales (Click & Grow, Xiaomi Smart Pot). La integración exitosa del asistente virtual Macety como elemento humanizador refuerza la propuesta de cercanía y educación continua, consolidando una ventaja competitiva sostenible en el mercado latinoamericano.

Madurez Técnica y Metodológica Consolidada del Equipo: La ejecución exitosa de tres sprints completos (Sprint 1: 19 SP, Sprint 2: 45 SP, Sprint 3: 48/50 SP = 96% completado) evidencia la capacidad del equipo para aplicar metodologías ágiles con rigor profesional. La gestión de código mediante GitFlow, documentación técnica con Swagger/OpenAPI, implementación de arquitectura de microservicios, configuración de CI/CD con GitHub Actions, despliegue en múltiples plataformas (Netlify, Azure, Firebase) y pruebas automatizadas con cobertura >70% demuestran madurez técnica excepcional. La coordinación efectiva entre desarrollo de firmware IoT, backend distribuido y frontend optimizado —validada mediante pruebas de campo de 48 horas en condiciones reales— asegura que MaceTy no solo es funcional, sino escalable, mantenible, seguro y listo para producción.

Impacto Medible y Trayectoria de Crecimiento Validada: El proyecto ha validado empíricamente un sólido problem-solution-product fit mediante múltiples fuentes de evidencia: entrevistas de needfinding, validaciones de prototipo, pruebas de usuario y métricas técnicas (latencia <2s, conectividad estable 48h, 96% de user stories completadas). Las conclusiones de las entrevistas —donde usuarios expresaron disposición de pago (S/200-300), intención de recomendación (100%) y solicitudes de features premium— confirman tracción de mercado real. La estrategia de negocio, fundamentada en un producto tecnológicamente robusto, identidad de marca clara (eco-tecnológica, cercana, educativa) y modelo de negocio mixto (hardware + freemium + B2B), posiciona a NaturaFy para capturar una cuota significativa del mercado de jardinería inteligente en Latinoamérica (mercado estimado en crecimiento del 15% CAGR 2023-2028). El ecosistema MaceTy, con su prototipo funcional desplegado, documentación completa y roadmap definido (notificaciones push, identificación de plantas por IA, integración con asistentes virtuales), establece las bases para escalar a producción comercial, captación de inversión y expansión geográfica regional.
